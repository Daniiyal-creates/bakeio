...
export default function WhatsappScreen() {
  ...
  const existing = connection.data;
  ...
  if (connection.isPending) return <Loader />;
  ...
  return (
    <>
      ...
      >
        ...
        <SectionCard title="How do you want to start?">
          <ChipPicker
            options={MODES}
            value={mode}
            onChange={setMode}
          ...
          <Typography type="body-sm" color="muted">
            {mode === 'test'
              ? 'Nothing is sent to WhatsApp. Use the “Try it” tab to chat with your assistant and get it right first.'
              : 'Real customer messages will be answered automatically. This needs a WhatsApp Business account approved by Meta.'}
          </Typography>
        </SectionCard>
        ...
        <Button size="lg" isDisabled={save.isPending} onPress={submit}>
          {save.isPending ? <Spinner size="sm" /> : null}
          <Button.Label>
            {existing ? 'Save changes' : mode === 'live' ? 'Connect number' : 'Save number'}
          </Button.Label>
        </Button>

        {existing ? (
          <Button variant="ghost" onPress={() => setDisconnectOpen(true)}>
            <Button.Label className="text-danger">Disconnect this number</Button.Label>
          </Button>
        ) : null}
        ...
        <ConfirmDialog
          ...
          description="Customer messages will stop arriving in Bakeio and nothing will be answered automatically. Your bakery information and past chats stay."
          confirmLabel="Disconnect"
          isPending={disconnect.isPending}
          onConfirm={() =>
            disconnect.mutate(existing!.id, { onSuccess: () => setDisconnectOpen(false) })
          }
        />
      </ScrollView>
    </>
